    ...
    try {
      const response = await traceAsync('auth_service_validate', async () => {
        return await axios.get(
          `${process.env.AUTH_SERVICE_URL || 'http://localhost:5001'}/api/auth/validate-token`,
          {
            headers: {
      ...
      req.owner = response.data.user._id;
 ...
 * Use this for critical operations like menu updates, status changes
 */
