...
import { AuthenticationError, AuthorizationError } from '@nova-health/types';
...
export async function authMiddleware(c: Context, next: Next) {
  try {
    ...
    // Note: JWT verification should be done using @nova-health/utils
    // This is a placeholder - actual implementation should use verifyToken from utils
    const decoded = JSON.parse(Buffer.from(token.split('.')[1], 'base64').toString()) as AuthContext;
