...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      className={cn("h-7 w-7 bg-muted z-[999] rounded-full", className)}
      data-sidebar="trigger"
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
    ...
    >
      ...
      {open ? (
        <ArrowLeftCircleIcon className="text-sidebar-foreground" />
      ) : (
        <ArrowRightCircleIcon className="text-sidebar-foreground" />
      )}
      <span className="sr-only">Toggle Sidebar</span>
    </Button>
  );
