...
async function main() {
  ...
  initTracing({ serviceName: 'ai-service', endpoint: config.observability?.otlpEndpoint });
  const metrics = new MetricsCollector('ai_service');
  ...
  const port = Number(process.env.AI_SERVICE_PORT) || 3015;
  await app.listen({ port, host: '0.0.0.0' });
  logger.info(`AI service listening on port ${port}`);
}

main().catch((err) => { console.error('Failed to start ai service:', err); process.exit(1); });
