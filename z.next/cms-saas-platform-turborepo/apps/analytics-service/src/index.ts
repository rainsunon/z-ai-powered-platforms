...
async function main() {
  ...
  initTracing({ serviceName: 'analytics-service', endpoint: config.observability?.otlpEndpoint });
  const metrics = new MetricsCollector('analytics_service');
  ...
  const port = Number(process.env.ANALYTICS_SERVICE_PORT) || 3007;
